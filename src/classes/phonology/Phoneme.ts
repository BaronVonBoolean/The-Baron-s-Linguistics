...
export class Phoneme {
  ...
  vectorizeConsonant():ConsonantVectors {
    const vectors:ConsonantVectors = [this.genera,'plosive', 'labiodental', 'voiceless'];

    if(plosives.includes(this.ipa)) vectors[1] = 'plosive';
    if(nasals.includes(this.ipa)) vectors[1] = 'nasal';
    if(approximant.includes(this.ipa)) vectors[1] = 'approximant';
    if(fricatives.includes(this.ipa)) vectors[1] = 'fricative';

    if(bilabials.includes(this.ipa)) vectors[2] = 'bilabial';
    if(labiodentals.includes(this.ipa)) vectors[2] = 'labiodental';
    if(alveolars.includes(this.ipa)) vectors[2] = 'alveolar';
    if(retroflexes.includes(this.ipa)) vectors[2] = 'retroflex';
    if(palatals.includes(this.ipa)) vectors[2] = 'palatal';
    if(velars.includes(this.ipa)) vectors[2] = 'velar';
    if(uvulars.includes(this.ipa)) vectors[2] = 'uvular';
    if(pharyngeals.includes(this.ipa)) vectors[2] = 'pharyngeal';
    if(glottals.includes(this.ipa)) vectors[2] = 'glottal';

    if(voiced.includes(this.ipa)) vectors[3] = 'voiced';
    if(!voiced.includes(this.ipa)) vectors[3] = 'voiceless';

    return vectors;
  }
  vectorizeVowel():VowelVectors {
    const vectors:VowelVectors = [this.genera, 'mid', 'central', 'unrounded'];
    
    if(closeVowels.includes(this.ipa)) vectors[1] = 'close'
    if(closeMidVowels.includes(this.ipa)) vectors[1] = 'close-mid'
    if(openMidVowels.includes(this.ipa)) vectors[1] = 'open-mid'
    if(openVowels.includes(this.ipa)) vectors[1] = 'open'

    if(frontVowels.includes(this.ipa)) vectors[2] = 'front'
    if(centralVowels.includes(this.ipa)) vectors[2] = 'central'
    if(backVowels.includes(this.ipa)) vectors[2] = 'back'

    if(roundedVowels.includes(this.ipa)) vectors[3] = 'rounded'
    if(!roundedVowels.includes(this.ipa)) vectors[3] = 'unrounded'
