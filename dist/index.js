"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const Annotator_1 = require("./classes/phonology/Annotator");
const compileVocabularyFromDir_1 = require("./processes/compileVocabularyFromDir");
const translateFile_1 = require("./processes/translateFile");
const compileMutationsFromDir_1 = require("./processes/compileMutationsFromDir");
const Morphology_1 = require("./classes/morphology/Morphology");
async function main() {
    const mutes = await (0, compileMutationsFromDir_1.compileMutationsFromDir)('./data/mutations');
    const vocab = await (0, compileVocabularyFromDir_1.compileVocabularyFromDir)('./data/dictionaries');
    const ann = new Annotator_1.Annotator(vocab);
    const morph = new Morphology_1.Morphology(vocab, ann);
    await morph.loadBoundMorphemesFromFile('/Users/ianculleton/Documents/node_projects/baron-linguistics/data/morphemes/morpheme.verbTense.txt');
    await morph.loadBoundMorphemesFromFile('/Users/ianculleton/Documents/node_projects/baron-linguistics/data/morphemes/morpheme.nounPlural.txt');
    const words = await ann.run('planting', mutes, 'ipa');
    if (!words)
        return;
    // this method updates the bound morphemes stored in the Morphology layer.  It is necessary for the 
    // Morphology layer to successfully detect the bound morphemes associated with a word.
    await morph.mutateBoundMorphemes(mutes);
    const characteristics = morph.getCharacteristics(words);
    console.log(characteristics);
    // console.log(morph.compose(decomposed))
    (0, translateFile_1.translateFile)(ann, './test-input-file.txt', mutes);
}
main();
